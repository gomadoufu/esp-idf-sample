...
clang環境でいくつかエラーが出たので、以下のサイトを参考に、プロジェクトルートにPythonファイルを追加 & CMakeListsを変更しています\\
